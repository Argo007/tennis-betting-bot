...
def main():
    ...
    for tour, repo in [("ATP","tennis_atp"), ("WTA","tennis_wta")]:
        for y in years:
            ...
            df = fetch_csv(url)
            df["tour"] = tour
            df = df[["tourney_date","winner_name","loser_name","surface","round","tourney_name","tour"]].dropna()
    ...
    def prior_prob(row):
        ...
        elif rd in ("qf","r16"): base += 0.02
        surf = str(row.get("surface","")).lower()
        if surf == "clay": base += 0.02
        elif surf == "grass": base -= 0.01
        base += float(rng.normal(0, 0.06))
        return float(np.clip(base, 0.52, 0.85))
    ...
    for tour in ("ATP","WTA"):
        ...
        for _,r in take.iterrows():
            p = prior_prob(r)                # prob winner beats loser
    ...
    out = pd.DataFrame(out_rows, columns=["date","player_a","player_b","odds_a","odds_b"]).sort_values("date")
