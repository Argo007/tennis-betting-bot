#!/usr/bin/env python3
"""
tennis-betting-bot unified runner (robust, merged rollup+commit)

Modes
- daily:   fetch -> build -> enrich -> picks -> dashboard/notify -> settle -> rollup+commit
- live:    live fetch -> build -> enrich -> picks -> dashboard/notify
- backtest:build -> backtest/report

Design
- Single source of truth for metrics (exported as env to all steps)
- Every intermediate writes a real file (header-only is OK)
- Engine short-circuits gracefully if there's no data
- End-of-run state rollup + git commit handled by state_rollup_commit.py
"""
...
# --------------------------------------------------------------------------------------
# Central metrics (tweak here, everything inherits via env)
# --------------------------------------------------------------------------------------
METRICS = {
    # Kelly & staking
    "KELLY_FRACTION": 0.50,
...
def step_engine_daily() -> None:
    edge_file = OUTPUTS / "edge_enriched.csv"
    if not csv_has_rows(edge_file):
        # write empty picks so later steps don't crash
        picks_live = REPO_ROOT / "picks_live.csv"
        RESULTS.mkdir(parents=True, exist_ok=True)
        header = "event_date,tournament,player,side,odds,prob,edge,stake\n"
        picks_live.write_text(header)
        (RESULTS / f"picks_{datetime.now().date().isoformat()}.csv").write_text(header)
        log("No enriched rows found; short-circuiting engine with empty picks (fallback).")
        return
...
# --------------------------------------------------------------------------------------
# Steps: state/settlement
# --------------------------------------------------------------------------------------
def step_state_rollup() -> None:
    # Settle trades (safe even if no log exists)
    if (SCRIPTS / "settle_trades.py").exists():
        run([PY, str(SCRIPTS / "settle_trades.py")])
    # Roll up bankroll + commit state/results (merged tool)
    run([PY, str(SCRIPTS / "state_rollup_commit.py")])
